...
public class EmiScreenshotRecorder {
	 ...
	 * {@code GameRenderer.mainRenderTarget} (it moved off {@code Minecraft} in 26.2) and takes its
	 * projection and scissor rectangles from the window render state, so both are pointed at the
	 * screenshot target for the duration of the render and restored afterwards. See D-R6 in the
	 * port decision log.
	 */
	private static void saveScreenshotInner(String path, int width, int height, Consumer<GuiGraphicsExtractor> renderer) {
		...
		if (guiRenderer == null || gameState == null) {
		...
		try {
			...
			((GameRendererAccessor) gameRenderer).emi$setMainRenderTarget(framebuffer);
			...
			((GameRendererAccessor) gameRenderer).emi$setMainRenderTarget(mainTarget);
