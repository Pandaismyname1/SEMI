...
public final class ContextNumbers {
	...
	public static float expectedValue(ContextIntProvider provider, Consumer<String> unhandled) {
		return expectedValue(provider, unhandled, 0);
	}
	...
	private static float expectedValue(ContextIntProvider provider, Consumer<String> unhandled, int depth) {
		if (depth > MAX_DEPTH) {
		...
		if (provider instanceof ConstantValue constant) {
			...
			for (Holder<ContextIntProvider> input : sum.inputs()) {
				total += expectedValue(input.value(), unhandled, depth + 1);
			}
			return total;
			...
			return extremum(minimum.inputs(), unhandled, depth + 1, true);
		} else if (provider instanceof Maximum maximum) {
			return extremum(maximum.inputs(), unhandled, depth + 1, false);
		} else if (provider instanceof Absolute absolute) {
			return Math.abs(expectedValue(absolute.input().value(), unhandled, depth + 1));
		} else if (provider instanceof Negate negate) {
			return -expectedValue(negate.input().value(), unhandled, depth + 1);
		} else if (provider instanceof FromFloat fromFloat) {
			return expectedValue(fromFloat.input().value(), unhandled, depth + 1);
		} else if (provider instanceof UniformGenerator uniform) {
			return (expectedValue(uniform.min().value(), unhandled, depth + 1)
				+ expectedValue(uniform.max().value(), unhandled, depth + 1)) / 2;
		} else if (provider instanceof Difference difference) {
			return expectedValue(difference.left().value(), unhandled, depth + 1)
				- expectedValue(difference.right().value(), unhandled, depth + 1);
		} else if (provider instanceof Quotient quotient) {
			float divisor = expectedValue(quotient.right().value(), unhandled, depth + 1);
			return divisor == 0 ? 0 : expectedValue(quotient.left().value(), unhandled, depth + 1) / divisor;
		} else if (provider instanceof FloorQuotient quotient) {
			float divisor = expectedValue(quotient.right().value(), unhandled, depth + 1);
			return divisor == 0 ? 0 : (float) Math.floor(expectedValue(quotient.left().value(), unhandled, depth + 1) / divisor);
		} else if (provider instanceof Modulus modulus) {
			float divisor = expectedValue(modulus.right().value(), unhandled, depth + 1);
			return divisor == 0 ? 0 : expectedValue(modulus.left().value(), unhandled, depth + 1) % divisor;
		} else if (provider instanceof FloorModulus modulus) {
			float divisor = expectedValue(modulus.right().value(), unhandled, depth + 1);
			if (divisor == 0) {
				return 0;
			}
			float dividend = expectedValue(modulus.left().value(), unhandled, depth + 1);
			return (float) (dividend - divisor * Math.floor(dividend / divisor));
		} else if (provider instanceof Power power) {
			return (float) Math.pow(expectedValue(power.base().value(), unhandled, depth + 1),
				expectedValue(power.exponent().value(), unhandled, depth + 1));
			...
			return expectedValue((ContextIntProvider) dispatcher.defaultValue().value(), unhandled, depth + 1);
		} else if (provider instanceof ConditionalProvider<?> conditional) {
			return expectedValue((ContextIntProvider) conditional.onFalse().value(), unhandled, depth + 1);
			...
			for (Weighted<Holder<ContextIntProvider>> entry : weighted.distribution().unwrap()) {
				total += entry.weight();
				sum += entry.weight() * expectedValue(entry.value().value(), unhandled, depth + 1);
			}
	...
	private static float expectedValue(ContextFloatProvider provider, Consumer<String> unhandled, int depth) {
		if (depth > MAX_DEPTH) {
		...
		if (provider instanceof net.minecraft.world.level.storage.loot.providers.number.floats.ConstantValue constant) {
			...
			return expectedValue(fromInt.input().value(), unhandled, depth + 1);
			...
			return floatExtremum(minimum.inputs(), unhandled, depth + 1, true);
		} else if (provider instanceof net.minecraft.world.level.storage.loot.providers.number.floats.Maximum maximum) {
			return floatExtremum(maximum.inputs(), unhandled, depth + 1, false);
			...
			for (Holder<ContextFloatProvider> input : length.inputs()) {
				float component = expectedValue(input.value(), unhandled, depth + 1);
			...
			return (float) Math.sin(expectedValue(sine.input().value(), unhandled, depth + 1));
		} else if (provider instanceof net.minecraft.world.level.storage.loot.providers.number.floats.Cosine cosine) {
			return (float) Math.cos(expectedValue(cosine.input().value(), unhandled, depth + 1));
			...
			return expectedValue((ContextFloatProvider) dispatcher.defaultValue().value(), unhandled, depth + 1);
		} else if (provider instanceof ConditionalProvider<?> conditional) {
			return expectedValue((ContextFloatProvider) conditional.onFalse().value(), unhandled, depth + 1);
	...
	private static float extremum(HolderSet<ContextIntProvider> inputs, Consumer<String> unhandled, int depth, boolean min) {
		Float best = null;
		for (Holder<ContextIntProvider> input : inputs) {
			float value = expectedValue(input.value(), unhandled, depth);
	...
	private static float floatExtremum(HolderSet<ContextFloatProvider> inputs, Consumer<String> unhandled, int depth, boolean min) {
		Float best = null;
	...
	private static String cycleId(@Nullable Identifier id, Object provider) {
		return typeId(id, provider) + " (nested more than " + MAX_DEPTH + " deep, probably a cycle)";
	}
}
